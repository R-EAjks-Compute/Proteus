  ...
  def apply(memorySize: BigInt)(implicit conf: Config): StaticPipeline = {
    ...
    pipeline.addPlugins(
      Seq(
        ...
        new rvp.memory.StaticMemoryBackbone,
        new rvp.memory.Fetcher(pipeline.fetch),
        ...
        new rvp.Conditional(Set(pipeline.execute)),
        new rvp.memory.Lsu(Set(pipeline.memory), Seq(pipeline.memory), pipeline.memory),
